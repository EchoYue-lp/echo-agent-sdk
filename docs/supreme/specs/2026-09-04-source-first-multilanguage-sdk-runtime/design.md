...
本设计保留 SDK Host、ACP 投影、扩展桥、资源生命周期和三语言适配的原始运行时决策，
但不再单独定义仓库归属或兼容阻断面：

- 仓库布局与长期文档 ownership 由
  [ADR 0001](../../../adr/0001-sdk-repository-boundary.md) 覆盖。下文第 17 节的
  `echo-agent/` 单仓布局是迁移前历史，不是当前目录合同；generic ACP adapter 留在
  framework，Host、protocol、contracts 和三语言 SDK 位于本仓。
- 下文“完整 Rust facade 是 SDK 承诺”的旧表述由
  [ADR 0031](../../../adr/0031-sdk-identity-governance-scope.md) 与
  [ADR 0032](../../../adr/0032-sdk-contract-scope-classification.md) 覆盖。只有明确接受的
  `external_contract` 是三语言阻断兼容面；完整 Rust public inventory 只作为非阻断漂移
  遥测，不自动扩大 SDK 承诺。

发生冲突时以上三份 ADR 优先；其余未冲突的运行时设计和历史验证仍有效。
