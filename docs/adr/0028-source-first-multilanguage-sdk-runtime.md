...
- Status: Accepted
- Date: 2026-09-04
- Owners: `echo-agent-sdk` maintainers and `echo-agent` ACP maintainers
...
Repository placement was amended by
[ADR 0001](./0001-sdk-repository-boundary.md): the generic ACP Agent adapter
remains in `echo-agent`, while the extension protocol, Host, contracts, and
language SDKs are maintained in this independent repository. The protocol and
runtime decisions below remain accepted unless ADR 0001 explicitly changes
their ownership.
...
16. Deliver the generic ACP adapter in `echo-agent`; deliver the Host, extension
    protocol, TypeScript SDK, Python SDK, and Java SDK as source in the
    independent `echo-agent-sdk` repository. Do not publish or download
    project-built binaries, npm packages, wheels, or JARs.
...
projection, and framework conformance tests belong in `echo-agent`. The
echo-agent extension contract, SDK Host, and language clients belong in
`echo-agent-sdk`. Both boundaries are complete without EKO and remain useful to
